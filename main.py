...
def addTo_available_tasks(val):
    ...
    val = val.decode()
    mycursor.execute(query, (val[2:],))
    mydb.commit()
    print(mycursor.rowcount, "record inserted.")
    mydb.close()

def read_available_tasks(val):
    mycursor, mydb = create_con_to_db()
...
def insert_db(val):
    ...
    val = val.decode()
    val = int(val[2:]) #val is the values from client - the task to insert
...
def update_record_table(val):
    ...
    val = val.decode()
    idx = val.find("#")  # '#' is the marker used to distingwish from old task to new
    val = (val[idx + 1:], int(val[idx-1])) #old value is second to send, inserted new value is first send
    mycursor.execute(query,val)
    mydb.commit()
